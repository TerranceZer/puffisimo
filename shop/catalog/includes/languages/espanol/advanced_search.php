<?php
/*
  $Id$

  osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Copyright (c) 2002 osCommerce

  Released under the GNU General Public License
*/

define('NAVBAR_TITLE_1', 'B&uacute;squeda Avanzada');
define('NAVBAR_TITLE_2', 'Resultados de la B&uacute;squeda');

define('HEADING_TITLE_1', 'B&uacute;squeda Avanzada');
define('HEADING_TITLE_2', 'Productos que satisfacen los criterios de la b&uacute;squeda');

define('HEADING_SEARCH_CRITERIA', 'Criterios de B&uacute;squeda');

define('TEXT_SEARCH_IN_DESCRIPTION', 'Buscar tambi&eacute;n en la descripci&oacute;n');
define('ENTRY_CATEGORIES', 'Categor&iacute;as:');
define('ENTRY_INCLUDE_SUBCATEGORIES', 'Incluir Subcategor&iacute;as');
define('ENTRY_MANUFACTURERS', 'Fabricantes:');
define('ENTRY_PRICE_FROM', 'Precio Desde:');
define('ENTRY_PRICE_TO', 'Precia Hasta:');
define('ENTRY_DATE_FROM', 'Fecha Desde:');
define('ENTRY_DATE_TO', 'Fecha Hasta:');

define('TEXT_SEARCH_HELP_LINK', '<u>Ayuda para la B&uacute;squeda</u> [?]');

define('TEXT_ALL_CATEGORIES', 'Todas las categor&iacute;as');
define('TEXT_ALL_MANUFACTURERS', 'Todos los fabricantes');

define('HEADING_SEARCH_HELP', 'Ayuda para la B&uacute;squeda');
define('TEXT_SEARCH_HELP', 'Las palabras clave pueden ser separados por las declaraciones AND y/o OR para un mayor control de los resultados de la b&uacute;squeda.<br /><br />Por ejemplo, <u>Billetera AND Tejido</u> generar&iacute;a un conjunto de resultados que contengan ambas palabras. Sin embargo, para <u>Cuero OR Cuerina</u>, el conjunto de resultados retornado podr&iacute;an contener las dos o una sola palabra.<br /><br />Resultados exactos pueden buscarse encerrando las palabras entre comillas.<br /><br />Por ejemplo, <u>"mochila de cuero"</u> la b&uacute;squeda dar&iacute;a como resultado el conjunto que tenga exactamente esa cadena.<br /><br />Los parentesis pueden ser utilizados para controlar el orden en el conjunto de resultados.<br /><br />Por ejemplo, <u>Lana and (Cuero or cuerina or "tejido")</u>.');
define('TEXT_CLOSE_WINDOW', '<u>Cerrar Ventana</u> [x]');

define('TABLE_HEADING_IMAGE', '');
define('TABLE_HEADING_MODEL', 'Modelo');
define('TABLE_HEADING_PRODUCTS', 'Nombre del Producto');
define('TABLE_HEADING_MANUFACTURER', 'Fabricante');
define('TABLE_HEADING_QUANTITY', 'Cantidad');
define('TABLE_HEADING_PRICE', 'Precio');
define('TABLE_HEADING_WEIGHT', 'Peso');
define('TABLE_HEADING_BUY_NOW', 'Comprar Ahora');

define('TEXT_NO_PRODUCTS', 'No hay productos que corresponden con los criterios de b&uacute;squeda.');

define('ERROR_AT_LEAST_ONE_INPUT', 'Al menos uno de los campos del formulario de b&uacute;squeda debe ser introducido.');
define('ERROR_INVALID_FROM_DATE', 'Fecha Desde no v&aacute;lido.');
define('ERROR_INVALID_TO_DATE', 'Fecha Hasta no v&aacute;lido.');
define('ERROR_TO_DATE_LESS_THAN_FROM_DATE', 'Fecha Hasta debe ser mayor o igual a la Fecha Desde.');
define('ERROR_PRICE_FROM_MUST_BE_NUM', 'Precio Desde debe ser un n&uacute;mero.');
define('ERROR_PRICE_TO_MUST_BE_NUM', 'Precio Hasta debe ser un n&uacute;mero.');
define('ERROR_PRICE_TO_LESS_THAN_PRICE_FROM', 'Precio Hasta debe ser mayor o igual al Precio Desde.');
define('ERROR_INVALID_KEYWORDS', 'Palabra(s) clave(s) no v&aacute;lida(s).');
?>